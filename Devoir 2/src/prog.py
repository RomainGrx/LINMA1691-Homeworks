...
def shortest_path_1(maze):
    ...
    possibilities = deque([(*Start, 0)]) # Liste les possibilit�s autour de chaques cases � tester avec leur longueur de chemin d�j� parcouru
    seen = {Start} # Liste toutes les cases o� nous sommes d�j� pass�s
    ...
    while possibilities:
        ...
        # Liste toutes les possibilit�s o� il n'y pas d'issues et reviens sur la boucle sans suite puisqu'il n'y a pas de chemins
        if(box == WALL):
            continue
...
def shortest_path_2(tasks, paths):
    ...
    while noeudres:
        ...
        # mettre à jour les moyens d'accéder à un noeud depuis running si plus court
        for i in neweight:
            if i[0]==running:
                if i[1] in visited:
